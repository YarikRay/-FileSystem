...
## 2) Логика работы итератора
   ...
   С целью подсчета размера вложенных директорий, реализуем функцию calc_size, которая на вход принимает путь до директории. Объявим переменную present_path, в которую положим путь до директории + "\\*", где * - шаблон, охватывающий все имена 
   объектов. Далее найдем первый файл в директории с помощью HANDLE hFind = FindFirstFileW(present_path.c_str(), &ffd);
   ...
   В переменную temp_path добавляем первый найденный файл, тем самым обновляя путь. С помощью условия if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) определяем является ли вновь найденный файл директорией. Если да, то к переменной 
   size прибавляем функцию calc_size(temp_path), реализуем, таким образом, рекурсивный подход внутри поддиректории. Если найденный файл не директория, то считаем размер отдельного файла и добавляем этот рамер в общую переменную size, тем
   ...
   Функция enumeration_method реализована схожим образом. Также осуществляется рекурсивный обход директорий с записей полученных значений в структурный вектор. 
   ...
   Далее эта переменная file аписывается в вектор с помощью info.push_back(file). После этого объявляется рекурсия для дальнейшего считывания файлов в поддиректориях. Объявляется так: enumeration_method(temp_path).
   Если же объект является файлом, то также в file.name кладется ffd.cFileName, в file.full_path сохраняется path_ent + L"\\" + ffd.cFileName. Переменная file.directory становится равной False, так как перед нами обычный файл.
   ...
   находим индекс точки после которого начинается расширение файла. Записываем этот индекс в переменную pos. В переменную file.extension записываем срез строки, в которой находится имя файла, начиная с точки до конца строки.
   Получаем расширение. 
